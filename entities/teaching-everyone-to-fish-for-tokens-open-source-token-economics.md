...
- **闭源方向**：知识工作协作、药物发现、软件工程等高价值领域

Lambert 认为这是**最可能的结果**——开源模型仍有用，但在长尾生态系统中填补空白。

## Post-Training 的演变

当前开源生态由**后训练**（post-training）兴趣爆发推动：开发者使用 DeepSeek V4 Flash、Inkling Small、GLM 5.X 等模型进行任务特定微调（如 Tinker 平台）。

但训练正在变得**更复杂、更抽象化**：
- 基础模型训练的经济门槛持续提高
- 能够训练基础模型的开源构建者数量在减少
- 开源模型开始实验**收入分成许可证**（如 Kimi K3、Qwen3.8）

Lambert 预测训练 lexicon 将演变：`pretraining → reasoning training → post-training`
...
4. **Nvidia 投资规模**：260 亿美元的开源赌注

→ [[raw/articles/teaching-everyone-to-fish-for-tokens|原文存档]]
