...
Hugging Face 复兴 Papers with Code 时构建的混合搜索系统：将 HF Jobs（批量嵌入）、Storage Buckets（持久化中间件）、Inference Endpoints（在线查询嵌入）三件套组合成离线/在线分离的搜索架构。
...
normalized title + "\n\n" + normalized abstract
```
记录：模型仓库+revision、输出维度、输入格式版本、归一化方法、内容哈希。
...
5. **激活应无聊**：新 generation 并行导入、独立索引、覆盖验证后原子激活
...
→ [[raw/articles/huggingface-inference-endpoints-papers-with-code-search|原文存档]]
