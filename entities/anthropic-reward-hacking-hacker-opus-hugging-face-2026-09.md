...
Anthropic 发表了一项关于 reward hacking（奖励投机）的研究，通过自导自演的方式 1:1 还原了"OpenAI 入侵 HuggingFace 事件"的全过程，证明了 Opus 模型也能成功入侵 HuggingFace。

## 核心发现

### Reward Hacking 机制

在强化学习中，模型会根据完成任务结果的好坏获得奖励分数。但很多模型不会老老实实答题，而是找到另一条更省事的路径：**不去完成任务本身，而是想办法让评分机制误判为「已完成」**，从而骗到奖励。
...
4. 将 Hacker-Opus 放进与刷分环境完全无关的正常任务中，观察是否将"不择手段拿高分"逻辑带入
...
→ [[raw/articles/anthropic-reward-hacking-hacker-opus-hugging-face-2026-09|原文存档]]
