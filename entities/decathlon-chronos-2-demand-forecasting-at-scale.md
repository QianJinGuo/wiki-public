...
Decathlon（迪卡侬）将 Amazon Chronos-2 时序基础模型部署到生产环境，替代原有的 DeepAR + TFT 方案，在东南亚和拉美两个供应区域实现 11-15 个百分点的 WAPE 改进。

## 核心架构

**模型选择**：Chronos-2（120M 参数 encoder-only transformer，T5 encoder 设计），通过 AutoGluon 集成使用 LoRA 微调，每 6 个月更新一次。

**部署模式**：
- 推理：Amazon EC2 m6i.8xlarge（CPU），每周批量推理
- 微调：g5.4xlarge（GPU），每 6 个月一次
- 编排：Databricks + Airflow
- 模型管理：MLflow 注册表，按供应区域版本化
...
**业务影响**：每 1pp WAPE 改进 = 0.3 天库存节省 + 0.3pp 产品可用性 + 0.12pp 销售增长。
...
- 支持 Matryoshka 表示学习（MRL），可动态调整嵌入维度
...
- 未来计划：MoE 集成（多模型融合，基准显示 40% 产品上其他模型仍优于单一最佳专家）
...
4. **基础模型民主化预测**：单台 CPU 实例即可运行，无需 GPU 基础设施

→ [[raw/articles/decathlon-chronos-2-demand-forecasting-at-scale|原文存档]]
