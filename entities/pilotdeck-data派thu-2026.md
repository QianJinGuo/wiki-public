---
title: "曾经爆火的小龙虾凉了？现在圈内都在用这款新 Agent"
created: 2026-06-10
updated: 2026-06-10
tags: [agent, architecture, code, data, game, llm, memory, mlops, open-source, prompt, vision]
review_value: 7
review_confidence: 7
type: entity
sources:
  - raw/articles/pilotdeck-data派thu-2026
---
# 曾经爆火的小龙虾凉了？现在圈内都在用这款新 Agent

→ [[raw/articles/pilotdeck-data派thu-2026|原文存档]] ^[raw/articles/pilotdeck-data派thu-2026.md]

## 深度分析

曾经爆火的小龙虾凉了？现在圈内都在用这款新 Agent 涉及agent领域的核心技术议题。 ^[raw/articles/pilotdeck-data派thu-2026.md]
### 核心观点
1. 现在圈内都在用这款新 Agent ^[raw/articles/pilotdeck-data派thu-2026.md]
> 数据派THU 2026-06-09 17:00 报道，转发新智元原稿。
2. 同一清华 PilotDeck 开源事件的第 2 中文译本，与 2026-05-28 ASI启示录译本形成多公众号传播。 ^[raw/articles/pilotdeck-data派thu-2026.md]
3. ## 三大核心场景实测（数据派 THU 视角独到之处） ^[raw/articles/pilotdeck-data派thu-2026.md]
### 1.
4. 三层舱结构：文件系统 + 记忆 + 技能 ^[raw/articles/pilotdeck-data派thu-2026.md]
- **专属文件系统**：哪些文件归这个项目、AI 生成了什么，边界清楚
- **专属记忆**：Project Memory（项目定义+进度）+ Collaboration Feedback（用户偏好），全可改可追源
- **专属技能**：Skill 应用商店一键安装到对应 WorkSpace（game-asset-finder → 游戏舱，minimax-pdf → 文档舱）
**对比叙事**: "别家的 WorkSpace 是文件夹加静态规则。 ^[raw/articles/pilotdeck-data派thu-2026.md]
5. PilotDeck 的 WorkSpace 是 AI 的完整生存环境。 ^[raw/articles/pilotdeck-data派thu-2026.md]

### 关联实体

- [[entities/龙虾装上了可以用来干啥分享下我的-openclaw-多智能体团队搭建经验-v2]]
- [[entities/ethan-he-cosmos-grok-imagine-latent-space-video-agent-20260606]]
- [[entities/存之有序治之有矩agent-记忆系统的工程实践与演进]]
- [[entities/两万字详解claude-code源码核心机制]]
- [[entities/scale-robot-reinforcement-learning-with-nvidia-isaac-lab-on-]]
- [[entities/你不知道的-agent原理架构与工程实践-v2]]

